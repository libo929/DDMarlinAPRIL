/**
 *  @file   DDMarlinArbor/src/DDArborPFAProcessor.cc
 * 
 *  @brief  Implementation of the arbor pfa new processor class.
 * 
 *  $Log: $
 */
...
DDArborPFAProcessor aDDArborPFAProcessor;
...
DDArborPFAProcessor::PandoraToLCEventMap DDArborPFAProcessor::m_pandoraToLCEventMap;

//------------------------------------------------------------------------------------------------------------------------------------------

DDArborPFAProcessor::DDArborPFAProcessor() :
    Processor("DDArborPFAProcessor")
{
...
void DDArborPFAProcessor::init()
{
    printParameters();
    try
    {
        streamlog_out(MESSAGE) << "DDArborPFAProcessor - Init" << std::endl;
        ...
        PANDORA_THROW_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, PandoraApi::ReadSettings(*m_pPandora, m_settings.m_arborSettingsXmlFile));
    }
    ...
    {
        streamlog_out(ERROR) << "Failed to initialize marlin arbor: std exception " << exception.what() << std::endl;
        throw exception;
    }
    catch (...)
    {
...
void DDArborPFAProcessor::processRunHeader(LCRunHeader */*pLCRunHeader*/)
{
}

//------------------------------------------------------------------------------------------------------------------------------------------

void DDArborPFAProcessor::processEvent(LCEvent *pLCEvent)
{
    try
    {
        streamlog_out(DEBUG) << "DDArborPFAProcessor - Run " << std::endl;
    ...
    {
        streamlog_out(ERROR) << "Marlin arbor failed to process event: std exception " << exception.what() << std::endl;
        throw exception;
    }
    catch (...)
    {
...
void DDArborPFAProcessor::check(LCEvent */*pLCEvent*/)
{
    streamlog_out(DEBUG) << "DDArborPFAProcessor - Check" << std::endl;
}

//------------------------------------------------------------------------------------------------------------------------------------------

void DDArborPFAProcessor::end()
{
    delete m_pPandora;
    ...
    streamlog_out(MESSAGE) << "DDArborPFAProcessor - End" << std::endl;
}

//------------------------------------------------------------------------------------------------------------------------------------------

const pandora::Pandora *DDArborPFAProcessor::GetPandora() const
{
    if (NULL == m_pPandora)
...
const EVENT::LCEvent *DDArborPFAProcessor::GetCurrentEvent(const pandora::Pandora *const pPandora)
{
...
pandora::StatusCode DDArborPFAProcessor::RegisterUserComponents() const
{
    ...
    PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, ArborContent::RegisterArborShowerProfilePlugin(*m_pPandora));
...
void DDArborPFAProcessor::ProcessSteeringFile()
{
    registerProcessorParameter("ArborSettingsXmlFile",
                            "The arbor settings xml file",
                            m_settings.m_arborSettingsXmlFile,
                            std::string());
    ...
    registerOutputCollection(LCIO::VERTEX,
                             ...
                             std::string("ArborPFAStartVertices"));
    ...
    registerProcessorParameter("ShouldFormTrackRelationships",
                            "Whether to form arbor track relationships using v0 and kink info",
...
/*
    ...
    registerProcessorParameter("SoftwareCompensationWeights",
                               "The 9 software compensation weights for Arbor energy correction",
    ...
    registerProcessorParameter("SoftwareCompensationEnergyDensityBins",
                               "The 10 software compensation density bins for Arbor energy correction",
    ...
    registerProcessorParameter("FinalEnergyDensityBin",
                               "The software compensation final energy density bins for Arbor energy correction",
    ...
    registerProcessorParameter("MaxClusterEnergyToApplySoftComp",
                               "The maximum hadronic energy to apply software compensation in Arbor energy correction",
    ...
    registerProcessorParameter("MinCleanHitEnergy",
                               "The minimum hit energy to apply ecal correction in Arbor energy correction",
    ...
    registerProcessorParameter("MinCleanHitEnergyFraction",
                               "The minimum hit energy fraction to apply ecal correction in Arbor energy correction",
    ...
    registerProcessorParameter("MinCleanCorrectedHitEnergy",
                               "The minimum correction to on ecal hit in Arbor energy correction",
...
void DDArborPFAProcessor::FinaliseSteeringParameters()
{
...
void DDArborPFAProcessor::Reset()
{
...
DDArborPFAProcessor::Settings::Settings() :
    m_innerBField(3.5f),
