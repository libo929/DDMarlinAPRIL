/**
 *  @file   DDMarlinAPRIL/src/DDAPRILPFAProcessor.cc
 * 
 *  @brief  Implementation of the april pfa new processor class.
 * 
 *  $Log: $
 */
...
DDAPRILPFAProcessor aDDAPRILPFAProcessor;
...
DDAPRILPFAProcessor::PandoraToLCEventMap DDAPRILPFAProcessor::m_pandoraToLCEventMap;

//------------------------------------------------------------------------------------------------------------------------------------------

DDAPRILPFAProcessor::DDAPRILPFAProcessor() :
    Processor("DDAPRILPFAProcessor")
{
    _description = "APRIL reconstructs clusters and particle flow objects";
    this->ProcessSteeringFile();
}

//------------------------------------------------------------------------------------------------------------------------------------------

void DDAPRILPFAProcessor::init()
{
    printParameters();
    try
    {
        streamlog_out(MESSAGE) << "DDAPRILPFAProcessor - Init" << std::endl;
        ...
        PANDORA_THROW_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, PandoraApi::ReadSettings(*m_pPandora, m_settings.m_aprilSettingsXmlFile));
    }
    catch (pandora::StatusCodeException &statusCodeException)
    {
        streamlog_out(ERROR) << "Failed to initialize marlin april: " << statusCodeException.ToString() << std::endl;
        throw statusCodeException;
    }
    catch (std::exception &exception)
    {
        streamlog_out(ERROR) << "Failed to initialize marlin april: std exception " << exception.what() << std::endl;
        throw exception;
    }
    catch (...)
    {
        streamlog_out(ERROR) << "Failed to initialize marlin april: unrecognized exception" << std::endl;
        throw;
    }
}
...
void DDAPRILPFAProcessor::processRunHeader(LCRunHeader */*pLCRunHeader*/)
{
}

//------------------------------------------------------------------------------------------------------------------------------------------

void DDAPRILPFAProcessor::processEvent(LCEvent *pLCEvent)
{
    try
    {
        streamlog_out(DEBUG) << "DDAPRILPFAProcessor - Run " << std::endl;
    ...
    {
        streamlog_out(ERROR) << "Marlin april failed to process event: " << statusCodeException.ToString() << std::endl;
        throw statusCodeException;
    }
    catch (EVENT::Exception &exception)
    {
        streamlog_out(ERROR) << "Marlin april failed to process event: lcio exception " << exception.what() << std::endl;
        throw exception;
    }
    catch (std::exception &exception)
    {
        streamlog_out(ERROR) << "Marlin april failed to process event: std exception " << exception.what() << std::endl;
        throw exception;
    }
    catch (...)
    {
        streamlog_out(ERROR) << "Marlin april failed to process event: unrecognized exception" << std::endl;
        throw;
    }
}
...
void DDAPRILPFAProcessor::check(LCEvent */*pLCEvent*/)
{
    streamlog_out(DEBUG) << "DDAPRILPFAProcessor - Check" << std::endl;
}

//------------------------------------------------------------------------------------------------------------------------------------------

void DDAPRILPFAProcessor::end()
{
    delete m_pPandora;
    ...
    streamlog_out(MESSAGE) << "DDAPRILPFAProcessor - End" << std::endl;
}

//------------------------------------------------------------------------------------------------------------------------------------------

const pandora::Pandora *DDAPRILPFAProcessor::GetPandora() const
{
    if (NULL == m_pPandora)
...
const EVENT::LCEvent *DDAPRILPFAProcessor::GetCurrentEvent(const pandora::Pandora *const pPandora)
{
...
pandora::StatusCode DDAPRILPFAProcessor::RegisterUserComponents() const
{
    ...
    PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, APRILContent::RegisterBFieldPlugin(*m_pPandora,
        m_settings.m_innerBField, m_settings.m_muonBarrelBField, m_settings.m_muonEndCapBField));

    PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, APRILContent::RegisterEnergyCorrections(*m_pPandora));
...
void DDAPRILPFAProcessor::ProcessSteeringFile()
{
    registerProcessorParameter("APRILSettingsXmlFile",
                            "The april settings xml file",
                            m_settings.m_aprilSettingsXmlFile,
                            std::string());
    ...
    registerOutputCollection(LCIO::VERTEX,
                             ...
                             std::string("APRILPFAStartVertices"));
    ...
    registerProcessorParameter("ShouldFormTrackRelationships",
                            "Whether to form april track relationships using v0 and kink info",
...
/*
    ...
    registerProcessorParameter("SoftwareCompensationWeights",
                               "The 9 software compensation weights for APRIL energy correction",
    ...
    registerProcessorParameter("SoftwareCompensationEnergyDensityBins",
                               "The 10 software compensation density bins for APRIL energy correction",
    ...
    registerProcessorParameter("FinalEnergyDensityBin",
                               "The software compensation final energy density bins for APRIL energy correction",
    ...
    registerProcessorParameter("MaxClusterEnergyToApplySoftComp",
                               "The maximum hadronic energy to apply software compensation in APRIL energy correction",
    ...
    registerProcessorParameter("MinCleanHitEnergy",
                               "The minimum hit energy to apply ecal correction in APRIL energy correction",
    ...
    registerProcessorParameter("MinCleanHitEnergyFraction",
                               "The minimum hit energy fraction to apply ecal correction in APRIL energy correction",
    ...
    registerProcessorParameter("MinCleanCorrectedHitEnergy",
                               "The minimum correction to on ecal hit in APRIL energy correction",
...
void DDAPRILPFAProcessor::FinaliseSteeringParameters()
{
...
void DDAPRILPFAProcessor::Reset()
{
...
DDAPRILPFAProcessor::Settings::Settings() :
    m_innerBField(3.5f),
